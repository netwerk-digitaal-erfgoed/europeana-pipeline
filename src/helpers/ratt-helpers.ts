...
import { Ratt, Middleware, MiddlewareList } from "@triply/ratt";
...
export function externalSPARQL(endpointKey: string, queryKey: string) {
  return addMwCallSiteToError(async function executeQuery(ctx, next) {
    const parser = new Parser();
    const url = ctx.getString(endpointKey).replace("<","").replace(">","");
    const query = ctx.getString(queryKey);
...
export function rattSPARQL(endpointKey: string, queryKey: string) {
  return addMwCallSiteToError(async function executeQuery(ctx, next) {
    let queryResult: IQueryResult;
    const engine = newEngine();
    const graph = ctx.app.defaultGraph;
    try {
      queryResult = await engine.query(ctx.getString(queryKey), {
        sources: [ctx.get(endpointKey)],
      });
    } catch (e) {
...
export function enrich_model(modelGraph: NamedNode) {
  ...
  return [
    mw.sparqlConstruct(
      Ratt.Source.TriplyDb.query("data-enrichment", "enrich-owl", {
    ...
    mw.sparqlConstruct(
      Ratt.Source.TriplyDb.query("data-enrichment", "enrich-taxonomy", {
        triplyDb: { url: triplydbApi },
      }),
      { toGraph: modelGraph }
    ),
    mw.sparqlConstruct(
      Ratt.Source.TriplyDb.query("data-enrichment", "enrich-skos-lex", {
        triplyDb: { url: triplydbApi },
      }),
      { toGraph: modelGraph }
    ),
    mw.sparqlConstruct(
      Ratt.Source.TriplyDb.query("data-enrichment", "enrich-node-shapes", {
        triplyDb: { url: triplydbApi },
      }),
      { toGraph: modelGraph }
    ),
    mw.sparqlConstruct(
      Ratt.Source.TriplyDb.query("data-enrichment", "enrich-property-shapes", {
        triplyDb: { url: triplydbApi },
      }),
      { toGraph: modelGraph }
    ),
    mw.sparqlConstruct(
      Ratt.Source.TriplyDb.query("data-enrichment", "enrich-sh-lex-1", {
        triplyDb: { url: triplydbApi },
      }),
      { toGraph: modelGraph }
    ),
    mw.sparqlConstruct(
      Ratt.Source.TriplyDb.query("data-enrichment", "enrich-sh-lex-2", {
        triplyDb: { url: triplydbApi },
      }),
      { toGraph: modelGraph }
    ),
    mw.sparqlConstruct(
      Ratt.Source.TriplyDb.query("data-enrichment", "enrich-lists", {
        triplyDb: { url: triplydbApi },
      }),
      { toGraph: modelGraph }
    ),
    mw.sparqlConstruct(
      Ratt.Source.TriplyDb.query("data-enrichment", "enrich-definitions", {
        triplyDb: { url: triplydbApi },
      }),
      { toGraph: modelGraph }
    ),
...
export function split(key: string, separator: string = ",") {
  return [
    mw.change({
      key: key,
      type: "unknown",
...
export function validate_model(reportDestination: any, reportGraph: NamedNode) {
  const scheme = "https";
  const host = "triplydb.com";
  const triplydbApi = `${scheme}://api.${host}`;
  const organization = "generic";
  ...
  const prefix_ = {
    graph: Ratt.prefixer(
      `${scheme}://${host}/${organization}/${dataset}/graph/`
    ),
  };
  const graph = {
...
export const sdo = {
  ...
  BroadcastFrequencySpecification: prefix.sdo(
  ...
  CharitableIncorporatedOrganization: prefix.sdo(
  ...
  // Represents EU Energy Efficiency Class A+ as defined in EU energy labeling regulations.
  EUEnergyEfficiencyCategoryA1Plus: prefix.sdo(
    "EUEnergyEfficiencyCategoryA1Plus"
  ),
  // Represents EU Energy Efficiency Class A++ as defined in EU energy labeling regulations.
  EUEnergyEfficiencyCategoryA2Plus: prefix.sdo(
    "EUEnergyEfficiencyCategoryA2Plus"
  ),
  // Represents EU Energy Efficiency Class A+++ as defined in EU energy labeling regulations.
  EUEnergyEfficiencyCategoryA3Plus: prefix.sdo(
  ...
  EducationalOccupationalCredential: prefix.sdo(
  ...
  EnergyStarEnergyEfficiencyEnumeration: prefix.sdo(
  ...
  HealthPlanCostSharingSpecification: prefix.sdo(
  ...
  MediaManipulationRatingEnumeration: prefix.sdo(
  ...
  MedicalGuidelineContraindication: prefix.sdo(
  ...
  MedicalObservationalStudyDesign: prefix.sdo(
  ...
  MerchantReturnFiniteReturnWindow: prefix.sdo(
  ...
  MerchantReturnPolicySeasonalOverride: prefix.sdo(
  ...
  OccupationalExperienceRequirements: prefix.sdo(
  ...
  ProductReturnFiniteReturnWindow: prefix.sdo(
  ...
  ReturnFeesCustomerResponsibility: prefix.sdo(
  ...
  ReturnLabelCustomerResponsibility: prefix.sdo(
  ...
  RisksOrComplicationsHealthAspect: prefix.sdo(
  ...
  UnincorporatedAssociationCharity: prefix.sdo(
  ...
  WearableMeasurementTypeEnumeration: prefix.sdo(
  ...
  customerRemorseReturnLabelSource: prefix.sdo(
    "customerRemorseReturnLabelSource"
  ),
  // The amount of shipping costs if a product is returned due to customer remorse. Applicable when property [[customerRemorseReturnFees]] equals [[ReturnShippingFees]].
  customerRemorseReturnShippingFeesAmount: prefix.sdo(
  ...
  feesAndCommissionsSpecification: prefix.sdo(
  ...
  itemDefectReturnShippingFeesAmount: prefix.sdo(
  ...
  maximumPhysicalAttendeeCapacity: prefix.sdo(
  ...
  missionCoveragePrioritiesPolicy: prefix.sdo(
  ...
  numberOfAvailableAccommodationUnits: prefix.sdo(
  ...
  originalMediaContextDescription: prefix.sdo(
  ...
  specialOpeningHoursSpecification: prefix.sdo(
...
export const sh = {
  ...
  LessThanOrEqualsConstraintComponent: prefix.sh(
  ...
  QualifiedMaxCountConstraintComponent: prefix.sh(
  ...
  QualifiedMinCountConstraintComponent: prefix.sh(
